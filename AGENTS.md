...
`disableExtensions` skips the 6 app extensions (share, widgets, notification service, etc.) — required for free Apple IDs (App ID limits) and avoids provisioning complexity. **Do not** add `disableProvisioningProfiles` here for device builds (it makes provisioning resolve to `None` and fails).

For simulator-only builds, pass `--disableProvisioningProfiles` on the command line (not in `local.bazelrc`).
